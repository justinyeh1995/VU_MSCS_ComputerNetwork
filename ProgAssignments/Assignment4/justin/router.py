...
def driver (args):
  ...
  try:
    ...
    bind_string = "tcp://" + route["bind"] + ":" + str (args.myport)
  ...
  try:
    # set our identity
    print ("router setting its identity: {}".format (route["bind"]))
    conn_sock.setsockopt (zmq.IDENTITY, bytes (route["bind"], "utf-8"))
  except zmq.ZMQError as err:
  ...
  try:
    ...
    nexthopport = 4444
    #if route["connect"] == "10.0.0.198":
    #    nexthopport = 5555
    connect_string = "tcp://" + route["connect"] + ":" + str (nexthopport)
